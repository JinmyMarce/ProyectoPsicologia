...
class AppointmentController extends Controller
{
            ...
            $appointments = Cita::with(['student', 'psychologist'])
                ...
                ->map(function ($appointment) {
                    return [
                        ...
                        'date' => $appointment->fecha ? (is_string($appointment->fecha) ? $appointment->fecha : $appointment->fecha->format('Y-m-d')) : null,
                        'time' => $appointment->hora ? (is_string($appointment->hora) ? $appointment->hora : $appointment->hora->format('H:i')) : null,
    ...
    'allergies' => 'nullable|string|max:1000',
            ...
            'motivo_consulta' => $request->reason ?? $request->motivo_consulta ?? '',
            'notas' => $request->notes,
            ...
            try {
                \App\Models\Notification::create([
                    ...
                    'message' => "El estudiante {$user->name} ({$user->email}) ha solicitado una cita para el día " . ($appointment->fecha ? (is_string($appointment->fecha) ? $appointment->fecha : $appointment->fecha->format('Y-m-d')) : 'N/A') . " a las " . ($appointment->hora ? (is_string($appointment->hora) ? $appointment->hora : $appointment->hora->format('H:i')) : 'N/A') . ". Motivo: {$appointment->motivo_consulta}. Por favor, revisa la solicitud y acepta o rechaza la cita desde tu panel de administración.",
                    'read' => false,
                    'data' => [
