...
export const AdminDashboard: React.FC = () => {
  ...
  return (
    <div className="container mx-auto p-0 pt-1 space-y-6 font-serif content-card" style={{fontFamily: 'Georgia, Times, serif'}}>
      {/* Título Principal */}
      <div className="text-center mb-6">
        <div className="inline-block px-20 py-4 bg-white border border-gray-200 rounded-lg shadow-md">
          <h1 className="text-2xl font-bold text-gray-800 tracking-tight mb-2">
            Panel de Administración - Sistema de Bienestar
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="content-card p-6 rounded-lg shadow-md border border-gray-200">
        ...
        <div className="content-card p-6 rounded-lg shadow-md border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800">{users.filter(u => u.role === 'admin' && u.active).length}</p>
        ...
        <div className="content-card p-6 rounded-lg shadow-md border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800">{users.filter(u => u.role === 'psychologist' && u.active).length}</p>
              <ul className="text-xs text-gray-500 mt-1">
        ...
        <div className="content-card p-6 rounded-lg shadow-md border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Pacientes Estudiantes Activos</p>
              <p className="text-2xl font-semibold text-gray-800">{users.filter(u => u.role === 'student' && u.active).length}</p>
      ...
      <div className="content-card rounded-lg shadow-md border border-gray-200 mt-8 p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-800">Últimos Usuarios Registrados</h2>
            <button
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="table-header">
              <th className="px-4 py-2 text-left font-semibold text-white">Nombre</th>
              <th className="px-4 py-2 text-left font-semibold text-white">Rol</th>
              <th className="px-4 py-2 text-left font-semibold text-white">Fecha Registro</th>
            </tr>
          </thead>
          <tbody>
            {users
              ...
              .slice(0, 5)
              .map((user) => (
                <tr key={user.id} className="table-row border-b last:border-b-0">
                  <td className="px-4 py-2 text-secondary">{user.name}</td>
                  <td className="px-4 py-2">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${user.role === 'psychologist' ? 'badge-secondary' : 'badge-primary'}`}>
                      {user.role === 'psychologist' ? 'Psicólogo' : 'Estudiante'}
                          </span>
                  </td>
                  <td className="px-4 py-2 text-secondary">{new Date(user.created_at).toLocaleDateString()}</td>
                </tr>
              ))}
          </tbody>
        </table>
      ...
      {error && (
        <div className="alert-error rounded-xl p-4 flex items-center space-x-2">
